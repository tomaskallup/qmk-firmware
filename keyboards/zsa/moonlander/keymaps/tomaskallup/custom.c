...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case PR_SC:
            if (record->event.pressed) {
                SEND_STRING(SS_RGUI(SS_RSFT(SS_TAP(X_PSCR))));
                return false;
            }
        case BSP_DEL: {
...
void matrix_init_user(void) {
    ...
    rgb_matrix_set_color_all(0x88, 0x88, 0x88);
}

layer_state_t layer_state_set_user(layer_state_t state) {
    switch (get_highest_layer(state)) {
        case CZE:
            rgb_matrix_set_color_all(0x00, 0xAA, 0xFF);
            break;
        case NUM:
            rgb_matrix_set_color_all(0xAA, 0x00, 0x00);
            break;
        case SYM:
            rgb_matrix_set_color_all(0x00, 0x00, 0xAA);
            break;
        case ARR:
            rgb_matrix_set_color_all(0x00, 0xAA, 0x00);
            break;
